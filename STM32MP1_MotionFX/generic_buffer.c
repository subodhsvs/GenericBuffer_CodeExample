...
static int64_t hw_timestamp, timestamp;
...
void print2byte(uint16_t input, struct iio_channel_info *info, float *fval)
{
		  ...
		  int num_channels)
{
	...
	timestamp = seconds * 1000000000 + ns;
	printf("\t%lld ns (Delta %lld ns)\n", timestamp, hw_timestamp - timestamp);
}
...
int main(int argc, char **argv)
{
	...
	for (j = 0; j < num_loops; j++) {
		...
		for (i = 0; i < read_size / scan_size; i++)
			process_scan(dev_dir_name, data + scan_size * i, channels,
				     num_channels);
	}
